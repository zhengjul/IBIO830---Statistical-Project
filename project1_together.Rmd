...
### Positional accuracy vs. latitude and longitude
Positional accuracy in INaturalist describes how far away the submitted observations are from the recorded location (longitude, latitude). This value represents meters away from the recorded location. Therefore, it is important for us to take a look at whether our longitude latitude information are accurate to where the genus Danaus was observed in nature and drop the outliers with extremely poor accuracy (AKA large positional accuracy values).
...
There are some outliers in our data, in particular in the datapoints that contain latitude or longitude greater than 30000 meteres away from actual observations. We decided to clean the data more thoroughly and remove any data points that are 15000 meters away from the recorded location.
...
plot(butterfly_data$positional_accuracy ~butterfly_data$longitude, 
     ...
     main = "Positional accuracy vs Longitude")
```
As we can see, the y-axis has changed from 30000 to 12000 meters, which is a significant improvement of maximum positional accuracy present in our dataset. We are more certain of our dataset's longitude and latitude values is reasonably close to actual observation location.
